...
def _optional_path(*candidates: Path) -> Path | None:
...
CAPTION_WEIGHTS_PATH = _optional_path(
...
CAPTIONS_TXT_PATH = _optional_path(
...
def load_vocab() -> dict[str, Any]:
    if VOCAB_PATH is None:
        raise FileNotFoundError("Missing vocab.pkl for captioning.")
...
def load_caption_system() -> dict[str, Any]:
    if CAPTION_WEIGHTS_PATH is None or VOCAB_PATH is None:
        raise FileNotFoundError("Captioning model files are not available.")
